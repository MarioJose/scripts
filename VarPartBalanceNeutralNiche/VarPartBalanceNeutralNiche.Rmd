# Variation partitioning and checking for balance between neutral and niche processes

This script implement partition variance following Anderson and Legendre (1999), Blanchet et al. (2008), Bocard and Legendre (2011), Dray et al. (2006), Legendre et al. (2012), Legendre and Gallagher (2001) and Peres-Neto and Legendre (2010). Balance between neutral and niche processes following Diniz-Filho et al. (2012).

[Danilo Rafael Mesquita Neves](http://lattes.cnpq.br/7825577355355814) and [Pedro V. Eisenlohr](http://pedroeisenlohr.webnode.com.br/) implemented variation partitioning and [Mario Jose Marques-Azevedo](https://github.com/mariojose) implemented balance between neutral and niche process.
...
To load yoyt data, you can use `read.table` command like that:
...
Check if your data is using "," as data separator character. If you is using other, like ";" change "sep" parameter of `read.table` command. You need to check if first column of your database file is a row names. Check your data using:
...
```{r, eval = FALSE}
# Convert to neighbours list. See Borcard et al. (2011) for details.
ll.nb <- tri2nb(ll)
...
Finishing data to partition variance, we will select significant MEMs for our species data using forward selection.
...
We will create a `spatial` variable with only MEMs selected by forwarding select. The same to environmental selected, `environment` variable. This variables will be used in variance partitioning.
...
<div style="text-align: center">
  ![Variation partitioning](varpart.png)
</div>
<br />

Now we can testing environmental and spatial significance.
...
## Balance between neutral and niche process

Diniz et al. (2012) proposed a framework to test if amount of variation predicted by pure spatial component [c] can be safely attributed to neutral process, rather than lack of spatially structured environmental predictor. If correlation of species abundance (R) hasn't correlation with species spatial map (M), then spatial component [c] can safely attributed do neutral process.

First, we need the function `correlogI` to do analysis. You can download the [`correlogI`](https://github.com/MarioJose/r-functions/tree/master/correlogI) function at [this link](https://raw.githubusercontent.com/MarioJose/r-functions/master/correlogI/correlogI.r). Save the function script at your work directory. Read [`correlogI`](https://github.com/MarioJose/r-functions/tree/master/correlogI) function page for more information.
